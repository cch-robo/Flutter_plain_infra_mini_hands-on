...
DIコンテナ(`CounterDouble`)のオブジェクトは、シングルトンにしているため、クラスの最上行に静的フラグ `isForbiddenInject`を追加します。  
...
_`step_4`では、_**依存元**のカウンタ機能はオリジナルと変わりませんが、**注入先**のカウント値の `getter`にログ出力が追加されています、_  
